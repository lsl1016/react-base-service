...
工具可见性 = **应用绑定的 caller 作用域**：该 caller 名下 + default 通用作用域的全部启用 http 工具（`route_values` 不参与 MCP 可见性；client/mcp 型工具不对外）。要收窄某个应用，就把它绑到工具更少的 caller（caller 即权限旋钮）；tools/call 时按名复核（客户端可能缓存旧清单）。

> 历史说明：v1.1 曾引入 mcp-server 式的按应用工具授权白名单（tblLlmMcpAppTool），v1.3 移除——与 caller 作用域语义重叠且体验割裂（管理台创建的应用默认绑 default，作用域无工具时授权清单恒为空）。
