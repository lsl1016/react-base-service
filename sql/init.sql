...
CREATE TABLE IF NOT EXISTS `tblLlmUserModel` (
    ...
    `api_url`             VARCHAR(512) NOT NULL DEFAULT '' COMMENT '自定义接入面base url(空=走api.yaml全局端点)',
    `context_tokens`      INT          NOT NULL DEFAULT 0  COMMENT '上下文容量token(0=回退模型目录)',
    `max_output_tokens`   INT          NOT NULL DEFAULT 0  COMMENT '单次最大输出token(0=回退端点/目录默认)',
    `support_thinking`    TINYINT      NOT NULL DEFAULT 0  COMMENT '能力开关:支持思考模式',
    `support_tools`       TINYINT      NOT NULL DEFAULT 0  COMMENT '能力开关:支持函数调用',
    `support_vision`      TINYINT      NOT NULL DEFAULT 0  COMMENT '能力开关:支持视觉输入',
...
CREATE TABLE IF NOT EXISTS `tblLlmReactRun` (
    ...
    `context_breakdown_json`    TEXT         COMMENT '最近一轮模型调用前上下文分类token估算(JSON:systemPrompt/messages/mcpTools/systemTools/skills/others)',
...
--     ADD COLUMN `context_breakdown_json` TEXT COMMENT '最近一轮模型调用前上下文分类token估算(JSON)' AFTER `cache_create_tokens`,
